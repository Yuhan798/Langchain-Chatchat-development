...
def search_docs_in_file(
    ...
    if kb is not None:
        if query:
            docs = kb.search_docs_in_file(query, file_name, top_k, score_threshold)
            data = [DocumentWithVSId(**x[0].dict(), score=x[1], id=x[0].metadata.get("id")) for x in docs]
        elif file_name or metadata:
            data = kb.list_docs(file_name=file_name, metadata=metadata)
            for d in data:
...
def _save_files_in_thread_bylink(links: List[str],
    ...
    def save_file_bylink(link: str, knowledge_base_name: str, override: bool) -> dict:
        '''
        保存单个文件。
        '''
        try:
            ...
            if response.status_code == 200:
                ...
                print("文件链接失败！")
                response_status = f"文件{filename}链接请求失败，链接地址{link}。"
                return dict(code=403, msg=response_status,data=data)
        except Exception as e:
            msg = f"{filename} 文件上传失败，报错信息为: {e}"
            logger.error(f'{e.__class__.__name__}: {msg}',
                         exc_info=e if Settings.kb_settings.log_verbose else None)
            return dict(code=500, msg=msg, data=data)
    print("get links is ", links)
    params = [{"link": link, "knowledge_base_name": knowledge_base_name, "override": override} for link in links]
    for result in run_in_thread_pool(save_file_bylink, params=params):
        yield result
    # for result in save_file_bylink(links[0],knowledge_base_name,override):
    #     yield result
...
def upload_docs_bylink(
        links: List[str] = Body(..., description="文件下载链接"),
        knowledge_base_name: str = Body(..., description="知识库名称"),
        override: bool = Body(False, description="覆盖已有文件"),
        to_vector_store: bool = Body(True, description="是否向量化上传的文件"),
        chunk_size: int = Body(512, description="知识库中单段文本最大长度"),
        chunk_overlap: int = Body(50, description="知识库中相邻文本重合长度"),
        zh_title_enhance: bool = Body(True, description="是否开启中文标题加强"),
        docs: str = Body("{}", description="自定义docs，以json字符串形式传递"),
        not_refresh_vs_cache: bool = Body(False, description="暂不保存向量库（用于FAISS）"),
        defult_vs_type = Body(Settings.kb_settings.DEFAULT_VS_TYPE, description="默认向量库"),
        embeddng_model = Body(Settings.model_settings.DEFAULT_EMBEDDING_MODEL, description="Embedding模型"),   
) -> BaseResponse:
    ...
    if kb is None:
        create_kb(
            knowledge_base_name=knowledge_base_name,
            vector_store_type=defult_vs_type,
            kb_info="",
            embed_model=embeddng_model
        )
        print(f"未找到知识库 {knowledge_base_name}，已创建新知识库！")
        kb = KBServiceFactory.get_service_by_name(knowledge_base_name)
    ...
    try:
        ...
        return BaseResponse(code=400, msg="docs 格式错误，应为合法的 JSON 字符串")

    failed_files = {}
    file_names = list(docs.keys())

    # 文件下载并保存到本地
    for result in _save_files_in_thread_bylink(
        links, knowledge_base_name=knowledge_base_name, override=override
    ):
        print("文件保存结果: ", result)
        filename = result["data"]["file_name"]
...
#         links: List[HttpUrl] = Field(..., description="上传文件链接，支持多个链接"),
#         override: bool = Field(False, description="覆盖已有文件"),
#         to_vector_store: bool = Field(True, description="上传文件后是否进行向量化"),
#         chunk_size: int = Field(CHUNK_SIZE, description="知识库中单段文本最大长度", alias="chunk_size"),  # 确保alias与CHUNK_SIZE常量匹配
#         chunk_overlap: int = Field(OVERLAP_SIZE, description="知识库中相邻文本重合长度", alias="chunk_overlap"),  # 确保alias与OVERLAP_SIZE常量匹配
#         zh_title_enhance: bool = Field(False, description="是否开启中文标题加强"),  # 假设ZH_TITLE_ENHANCE是False
#         docs: Json=Field({},description="自定义的docs，需要转为json字符串",examples=[{"text.txt":[Document(page_content="custom doc")]}]),
#         not_refresh_vs_cache: bool = Field(False, description="暂不保存向量库（用于FAISS）"),
# ) -> BaseResponse:
#     if not validate_kb_name(DocumentUploadRequest.knowledge_base_name):
#         return BaseResponse(code=403, msg="Don't attack me")

#     kb = KBServiceFactory.get_service_by_name(DocumentUploadRequest.knowledge_base_name)
#     if kb is None:
#         create_kb(knowledge_base_name=DocumentUploadRequest.knowledge_base_name,vector_store_type=DEFAULT_VS_TYPE,embed_model=EMBEDDING_MODEL)
#         print("No kb named {},creating new kb !".format(DocumentUploadRequest.knowledge_base_name))
#         kb = KBServiceFactory.get_service_by_name(DocumentUploadRequest.knowledge_base_name)
#         # return BaseResponse(code=404, msg=f"未找到知识库 {DocumentUploadRequest.knowledge_base_name}")

#     failed_files = {}
#     file_names = list(DocumentUploadRequest.docs.keys())

#     for result in _save_files_in_thread_bylink(DocumentUploadRequest.links, knowledge_base_name=DocumentUploadRequest.knowledge_base_name, override=DocumentUploadRequest.override):
...
#     if result["code"] != 200:
#         return BaseResponse(code=404, msg="文件存储异常", data={"failed_files": failed_files})

#     if DocumentUploadRequest.to_vector_store:
#         result = update_docs(
#             knowledge_base_name=DocumentUploadRequest.knowledge_base_name,
#             file_names=file_names,
#             override_custom_docs=True,
#             chunk_size=DocumentUploadRequest.chunk_size,
#             chunk_overlap=DocumentUploadRequest.chunk_overlap,
#             zh_title_enhance=DocumentUploadRequest.zh_title_enhance,
#             docs=DocumentUploadRequest.docs,
#             not_refresh_vs_cache=True,
#         )
#         failed_files.update(result.data["failed_files"])
#         if not DocumentUploadRequest.not_refresh_vs_cache:
#             kb.save_vector_store()
