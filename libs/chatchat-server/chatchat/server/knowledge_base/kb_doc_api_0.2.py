...
from server.knowledge_base.utils import (validate_kb_name, list_files_from_folder, get_file_path,
                                         files2docs_in_thread, KnowledgeFile,DocumentUploadRequest)
...
def search_docs_in_file(
    ...
    if kb is not None:
        if query:
            docs = kb.search_docs_in_file(query, top_k, score_threshold,file_name)
            data = [DocumentWithVSId(**x[0].dict(), score=x[1], id=x[0].metadata.get("id")) for x in docs]
        elif file_name or metadata:
            data = kb.list_docs(file_name=file_name, metadata=metadata)
            for d in data:
    ...
    def save_file_bylink(link: str, knowledge_base_name: str, override: bool) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                print("Get file connection succeed!!!")
                if (os.path.isfile(file_path)
                        and not override
                ):
                    print("已检测到上传文件已经存在！！")
